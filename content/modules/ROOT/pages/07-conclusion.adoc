...
. **AI skills accelerate authoring**: `/showroom:create-lab` and `/showroom:create-demo` generate production-quality content in minutes, following Red Hat style and accessibility standards automatically
. **Two content types, one repo**: Labs for hands-on learning, demos for presenter-led walkthroughs -- both live in the same Showroom repository
...
. Create a new Showroom repo: `gh repo create rhpds/<your-product>-showroom --template rhpds/showroom_template_nookbag --public --clone`
. Clean and configure with Claude Code
. Run `/showroom:create-lab` with your real product documentation as reference material
. Optionally add a demo with `/showroom:create-demo`
. Publish and share with the field team
